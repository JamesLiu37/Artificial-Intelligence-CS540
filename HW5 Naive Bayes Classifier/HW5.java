...
public class HW5 {
	...
	private static double[] evaluate(Label[] yTrue, Label[] yPred) {
		...
		double numCorrectPredicted = 0, truePos = 0, numPredictedPos = 0, numActualPos = 0;
		int numInstances = yTrue.length;
		for (int i = 0; i < numInstances; i++){
			printOut(i + 1, yTrue[i], yPred[i]);
			if (yPred[i] == Label.Positive){
				numPredictedPos++;
				if (yTrue[i] == Label.Positive){ // true positive
					numCorrectPredicted++; truePos++; numActualPos++;
				}					
			}
			else // predicted negative class
				if (yTrue[i] == Label.Negative)
					numCorrectPredicted++;
				else
					numActualPos++;
		}
		
		scores[0] = round3( numCorrectPredicted / numInstances );
		scores[1] = round3( truePos / numPredictedPos );
		scores[2] = round3( truePos / numActualPos );		
